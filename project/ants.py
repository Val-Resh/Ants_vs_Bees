...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        ant = self.place.ant
        return ant != None and ant.blocks_path 
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        nearest_place = None
        count = 0
        while nearest_place == None and this_place != hive and count <= self.max_range:
            if len(this_place.bees) == 0 or count < self.min_range:
                this_place = this_place.entrance
                count += 1
            else:
                nearest_place = this_place

        return None if nearest_place == None else random_or_none(nearest_place.bees)
    ...
    def action(self, colony):
        ...
        if self.id != 1:
            ...
            while current_place.entrance is not colony.hive:
                if current_place.ant is not None and current_place.ant not in QueenAnt.doubled_ants:
